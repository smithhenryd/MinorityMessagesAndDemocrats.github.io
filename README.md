...
- [Week 8](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#xi-week-8-final-week-prior-to-second-evaluation)
- [Week 7](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#x-week-7)
- [Week 6](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#viv-week-6)
- [Week 5](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#viii-week-5-first-evaluation-and-beginning-of-second-coding-period)

**First Coding Period**:
- [Week 4](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#vii-week-4-final-week-prior-to-first-evaluation)
- [Week 3](https://smithhenryd.github.io/UnderrepresentedMessagesAndDemocrats.github.io/#vi-week-3)
...
Across all four appeal variables, the distributions of predicted scores are right-skewed; therefore, uniformly sampling from the support of scores will result in a sample with a greater proportion of high scores than in the underlying distribution of all Facebook images. As mentioned previously, because the goal of the second sample is to select images with a higher probability of positive annotation with the four appeal variables, this is the desired result. Notice that some of the predicted appeal variable score distributions, such as that of 'Asian_appeal', are more strongly right-skewed. As a result, the distribution of appeal scores for the sample images is not as uniform as desired (once all the images with high scores were selected, the distance from the sampled score to the image with the closest score was much larger). Fortunately, this does not pose a problem due to the aforementioned desire for more positively-annotated images. 

Once the samples of n<sub>2i</sub> = 1,000 (1 ≤ i ≤ 4) images were compiled for each of the four appeal variables, the images were combined and duplicates were removed. The resulting sample of n<sub>2</sub> = 3,886 images were formatted into an input csv file for manual annotation by Amazon Mechanical Turk. Each image pair (politician image, Facebook image, and caption) will be annotated according to the [finalized annotation scheme](https://github.com/smithhenryd/GSoC_2020_UnderrepresentedMessagesAndDemocrats-/blob/master/imgs_data/Images%20Annotation%20Scheme_Final.pdf). 
...
While this does not provide us with an absolute categorization of the content of images annotated with the four appeal variables, we can glean general ideas or concepts. First, it seems that many of the 'Black_appeal' images shared by Democratic politicians are black-and-white photographs paying homage to the U.S. Civil Rights Movement. Moreover, the labels may suggest that 'Hispanic_appeal' and 'Asian_appeal' images share content related to Hispanic and Asian culture and traditions, respectively. Lastly, the four labels correlated with 'LGBTQ+\_appeal' clearly capture Democratic politicians' outreach to the LGBTQ+ community through pride events and festivals. 

With the second evaluation period approaching, Dr. Kunwoo Park and I have been preparing the second sample of Facebook images and captions for annotation by Amazon Mechanical Turk. So far, Dr. Park trained the image and text-based machine learning models (residual and fully-connected neural network, respectively) using the set of existing annotations and applied the model to the entire dataset of images and captions. Combining the results from the two models, we observed [ROC AUC values](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc) and [F-1 scores](https://en.wikipedia.org/wiki/F1_score) that were higher than when the model was trained on my annotations alone. The next step is for Dr. Park and I use the distribution of scores from the combined ML models (from 0 to 1) to generate a new, non-random sample of images for annotation; for each appeal variable, we will sample ~1,000 images from the distribution of scores.
...
The finalized scheme (which will be used for annotation) can be found [here](https://github.com/smithhenryd/GSoC_2020_UnderrepresentedMessagesAndDemocrats-/blob/master/imgs_data/Images%20Annotation%20Scheme_Final.pdf).
