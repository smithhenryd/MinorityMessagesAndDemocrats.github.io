...
With the second evaluation period approaching, Dr. Kunwoo Park and I have been preparing the second sample of Facebook images and captions for annotation by Amazon Mechanical Turk. So far, Dr. Park trained the image and text-based machine learning models (residual and fully-connected neural network, respectively) using the set of existing annotations and applied the model to the entire dataset of images and captions. Combining the results, we observed [ROC AUC values](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc) and [F-1 scores](https://en.wikipedia.org/wiki/F1_score) that were higher than when the model was trained on my annotations alone. 
